...
!   enddo
  ...
  udx=1./(dlx+dlx)
  do j=1,ny
     dfi(1,j)=udx*(phi(2,j)-phi(1,j))
     do i=2,nx-1
        dfi(i,j)=udx*(phi(i+1,j)-phi(i-1,j))
     enddo
     dfi(nx,j)=udx*(phi(nx,j)-phi(nx-1,j))
  enddo
...
!############################################
  ...
  udx=1./(dlx*dlx)
  do j=1,ny
     dfi(1,j)=udx*(-2*phi(2,j)+phi(1,j)+phi(3,j))
     do i=2,nx-1
        dfi(i,j)=udx*(phi(i+1,j)-(phi(i,j)+phi(i,j))&
             +phi(i-1,j))
     enddo
     dfi(nx,j)=udx*(-2*phi(nx-1,j)+phi(nx,j)+phi(nx-2,j))
  enddo
...
subroutine boundary(uuu,vvv,rho,eee,pre,tmp,rou,rov,roe,nx,ny,&
   ...
   do j=1,ny
      i=nx
      uuu(i,j)=uu0
      rho(i,j)= rho(i,j) - uu0*(dlt/dlx)*(rho(i,j) - rho(i-1,j))
      rou(i,j)= rou(i,j) - uu0*(dlt/dlx)*(rou(i,j) - rou(i-1,j))
      rov(i,j)= rov(i,j) - uu0*(dlt/dlx)*(rov(i,j) - rov(i-1,j))
      roe(i,j)= roe(i,j) - uu0*(dlt/dlx)*(roe(i,j) - roe(i-1,j))
      scp(i,j)= scp(i,j) - uu0*(dlt/dlx)*(scp(i,j) - scp(i-1,j))
